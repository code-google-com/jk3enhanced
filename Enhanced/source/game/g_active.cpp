...
public:
	...
	{
		value = newValue.AsChar();
		return *this;
	}

	qboolean operator+=(const char & str)
	{
		Q_strcat(value, 1024, (char*)&str);
		return qtrue;
	}

	qboolean operator+=(const string & str)
	{
		Q_strcat(value, 1024, str.AsChar());
		return qtrue;
	}
	...
	qboolean StartsWith(const char *str)
	{
	...
	qboolean EndsWith(const char *str)
	{
		int z = 0;

		if(strlen(str) > strlen(value)) return qfalse;

		for(int i = strlen(value) - strlen(str); i < strlen(str); i++)
		{
			if(value[i] != str[z++])
				return qfalse;
		}
	...
	qboolean Contains(const char*str)
	{
		if(strlen(str) > strlen(value)) return qfalse;
